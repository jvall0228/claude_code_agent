...
.PHONY: setup start stop restart status health install uninstall

# ---------------------------------------------------------------------------
# Setup — one-command bootstrap from clone to running daemon
# ---------------------------------------------------------------------------

setup:
	@$(REPO_ROOT)setup.sh --agent-name $(INSTANCE)
